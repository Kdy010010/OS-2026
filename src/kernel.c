...
/* ── 메모리 레이아웃 ──────────────────────────────────────
 * RTL8139 RX ring : 0x500000 (64KB+16)
 * TX buffers      : 0x510000 (4 × 2KB)
 * Scratch/packet  : 0x520000 (32KB)
 * ────────────────────────────────────────────────────────*/
#define NIC_RX_BUF    ((volatile uint8_t*)0x500000)
#define NIC_TX_BUF0   ((volatile uint8_t*)0x510000)
#define NIC_SCRATCH   ((uint8_t*)0x520000)
...
static void nic_send(const uint8_t *pkt, int len){
    ...
    for(int t=0;t<200000;t++){
        ...
        if(tsd & RTL_TX_OK){ nic_last_tx_ok=1; break; }
        if(tsd & 0x8000){ break; } /* TX error bit */
    }
...
static int nic_recv(uint8_t *pkt, int maxlen){
    ...
    if(plen<14||plen>1514) { nic_rx_ptr+=((plen+4+3)&~3); nic_outw(RTL_CAPR,(uint16_t)(nic_rx_ptr-0x10)); return 0; }
...
static void net_handle_pkt(const uint8_t *pkt, int len){
    ...
    if(proto==17){
        ...
        int payload_len=len-(14+ip_hlen+tcp_hlen2);
...
static int http_get(const char *url, char *out_buf, int out_max){
    ...
    char ip_str[20]; int isi=0;
    uint8_t b0=(uint8_t)(server_ip>>24), b1=(uint8_t)(server_ip>>16),
            b2=(uint8_t)(server_ip>>8),  b3=(uint8_t)server_ip;
    /* 간단한 IP 출력 */
    void ip_nibble(uint8_t b){ char tb[4]; int ti=0;
        if(!b){ip_str[isi++]='0';}else{while(b){tb[ti++]='0'+b%10;b/=10;} while(ti--) ip_str[isi++]=tb[ti+1];}
    }
    (void)ip_nibble; /* 람다 불가 – 인라인으로 처리 */
    /* IP 출력을 puts로 직접 처리 */
    char ipbuf2[40]; int ibi=0;
    uint8_t ibs[4]={(uint8_t)(server_ip>>24),(uint8_t)(server_ip>>16),(uint8_t)(server_ip>>8),(uint8_t)server_ip};
    for(int q2=0;q2<4;q2++){
        uint8_t bv=ibs[q2]; if(!bv){ipbuf2[ibi++]='0';}
        else{ char tb[4];int ti=0;while(bv){tb[ti++]='0'+bv%10;bv/=10;} while(ti--) ipbuf2[ibi++]=tb[ti+1]; }
        if(q2<3) ipbuf2[ibi++]='.';
    }
    ipbuf2[ibi]=0;
    puts(ipbuf2); puts("\n");
    (void)isi;(void)ip_str;(void)b0;(void)b1;(void)b2;(void)b3;
    ...
    int ri2=0;
    /* GET /path HTTP/1.0\r\n */
    const char *ms="GET "; while(*ms) req[ri2++]=*ms++;
...
static void bmp_oval(bmp16_t *b, int cx,int cy,int rx,int ry){
    ...
    for(int yy=-ry;yy<=ry;yy++) for(int xx=-rx;xx<=rx;xx++){
        long A=(long)xx*ry, B=(long)yy*rx, R=(long)rx*ry;
        if(A*A+B*B > R*R) continue;
        long A2=(long)(xx+1)*ry, B2=(long)(yy+1)*rx;
        if(A2*A2+B*B>R*R || A*A+B2*B2>R*R) bmp_setpx(b,cx+xx,cy+yy);
    }
}
    ...
    switch(eax){
        ...
        case 20: {
            ...
            for(int i=0;i<DIR_ENTRIES;i++){
                ...
                if(!strcmp(de->name,nm)){ memset(de,0,sizeof(dirent_t)); ata_write28(cur_dir,DIR_SECTORS,dirbuf); return 1; }
            }
            return 0;
        }
        case 21: {
...
static inline uint8_t gpx(int x,int y){
    if((unsigned)x<SCR_W&&(unsigned)y<SCR_H) return FB[y*SCR_W+x];
    return 0;
}
...
static void gui_run(void){
    ...
    for(;;){
        ...
        if(k==0){
            if(ck_upd){ cur_erase(); redraw_clock(); cur_draw(); }
            else if(need){ cur_erase(); draw_all(); cur_draw(); }
            continue;
        }
        need=1;
